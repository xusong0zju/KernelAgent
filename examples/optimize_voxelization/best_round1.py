...
def _voxel_accumulate_faster(
    ...
    """
    Accumulate points directly from a (N,4) tensor.
    ...
    # convert to int32 – the voxel grid size fits in 32 bits
    xi = x.to(tl.int32)
    yi = y.to(tl.int32)
    zi = z.to(tl.int32)

    # clamp to valid range [0, size-1] (use maximum/minimum, not clamp)
